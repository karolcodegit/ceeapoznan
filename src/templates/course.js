  ...
  return (
    <div className="py-20 max-w-6xl max-md:mx-auto gap-32 flex flex-col xl:flex-row justify-between">
      <div className=" max-w-2xl">
            ...
            {course.daycourse.length === 0 ? (
              ...
              course.daycourse.map((day, index) => (
                <div key={index} className="my-8 space-y-4">
                  ...
                  <span className="text-gray-500 dark:text-gray-200">{day.date}</span>
                  <ul className="space-y-2">
                    {day.event.map((event, eventIndex) => (
                      <li key={eventIndex} className="py-3 text-gray-600 dark:text-gray-200 ">
        ...
        {(course.detailedInformationAboutTheCourse || course.detailedInformationAboutTheHotel) && (
        <div className="flex justify-start mt-8">
          <Button href={`/kursy/${slugify(course.nameCourse)}/szczegoly`}>Szczegółowe informacje o kursie</Button>
        </div>
      )}
      </div>
      <SideInfoPanel
